...
class MapGraphTest {
	...
	val graph = MapGraph.readFromAdjacentListFile(Path.fromString("src/main/resources/graph1.txt"))
	...
	@Test def testRandomCutCount = {
		for(i <- 1 to 10){
			Console.println(MapGraph.randomCutCount(graph1))
		}
	}
	...
	@Test def findMinCutCount = {
		...
		for(i <- 1 to 20){
			count = Math.min(MapGraph.randomCutCount(graph),count)
		}
		Console.println(count)
	}

  @Test def readSCCGraph = {
    val graph = MapGraph.readFromEdgeFile(Path.fromString("src/main/resources/SCC.txt"))
    assert(graph!=null)
